...
(function($) {
  function drags(dragElement, resizeElement, container) {
    var IsRTL =  $('body').hasClass('rtl_true'),
       pos_css = IsRTL ? 'right' : 'left';
    ...
    dragElement.on('mousedown.ba-events touchstart.ba-events', function(e) {
      ...
      dragElement.parents().on("mousemove.ba-events touchmove.ba-events", function(e) {
        ...
        widthValue =  IsRTL ? 100-widthValue+'%' : widthValue+'%';
...
//     cur.find('.resize img').css('width', width);
    ...
    drags(cur.find('.handlebeaf'), cur.find('.resizebeaf'), cur);
   ...
   */
    geckoShopify.ThemeT4Compare = function () {
      ...
      var txt_view = nt_settings.added_text_cp,
          cp_ntjs = $('#cp_ntjs'),
          limit = 6,
          pp_cp_true = body.hasClass('pp_cp_true'),
          Iscompare_page = ($('#shopify-section-compare_page .empty_cart_page').length > 0),
          $compareListBottom = $('.t4s-compare-list-bottom'),
      ...
      body.on('click', '.cpt4_added,.js_link_cp', function (e) {
        ...
        $ld.trigger( "ld_bar_star" );
        if (pp_cp_true && !Iscompare_page && !$(this).hasClass('t4s-compare-view')) {
          _ShowPopupCompare(0);
        } else {
          setTimeout(function(){ $ld.trigger( "ld_bar_end" ); }, 300);
      ...
      body.on('click', '.cpt4_remve', function (e) {
        ...
        var arrls = ls.split(','),
          ...
          localStorage.setItem('nt_cp', arrls.toString());
           ...
           $('.compare_id_'+dt_id).remove();
           //$('.compare_id_'+dt_id).hide('100', function(){ $('.compare_id_'+dt_id).remove(); });
           geckoShopify.compareUpdate(1);
           $('.cpt4_added[data-id="'+dt_id+'"]').removeClass('cpt4_added').addClass('compare_add');
           if (arrls.toString() == '' && !_this.hasClass('compare_remove_item')) {
             window.location.href = cp_ntjs.attr('href');
           } else if (arrls.toString() == '') {
            _ClosePopupCompare();
           } 
           // else {
           //  _ShowPopupCompare();
      ...
      // remove all compare
      body.on('click', '[data-cpt4_remve_all]', function (e) {
        e.preventDefault();
        localStorage.setItem('nt_cp','');
        geckoShopify.compareUpdate(1);
        $('.cpt4_added').addClass('compare_add').removeClass('cpt4_added');
        _ClosePopupCompare();
      });

      // t4s-close-compare
      body.on('click touchstart', '.t4s-close-compare,.mask-overlay.cp_pp_t4s', function (e) {
        e.preventDefault();
        _ClosePopupCompare();
      });
      
      function _ShowPopupCompare(bl) {
        if (!pp_cp_true) return;
        if ($('.t4s-compare-list').length == 0 || bl) {
          fetch(cp_ntjs.attr('href').replace('view=compe','view=cp_pp')).then(function(response) {
              return response.text();
            }).then(function(comparePopupHTML) {
              $ld.trigger( "ld_bar_end" );
              if (comparePopupHTML.trim() === '') {
                return;
              }
              //console.log(comparePopupHTML);
             $compareListBottom.addClass('t4s-active').html(comparePopupHTML);
             $maskOverlay.addClass('mask_opened cp_pp_t4s');
             $html_body.addClass('hside_opened');
            });
        } else {
            $ld.trigger( "ld_bar_end" );
            $compareListBottom.addClass('t4s-active');
            $maskOverlay.addClass('mask_opened cp_pp_t4s');
            $html_body.addClass('hside_opened');
        }
      };
      
      function _ClosePopupCompare() {
        if (!pp_cp_true) return;
        $compareListBottom.removeClass('t4s-active')
        $maskOverlay.removeClass('mask_opened cp_pp_t4s');
        $html_body.removeClass('hside_opened');
      };
    ...
    geckoShopify.currencyForm = function () {
      ...
      if ( $('#CurrencyLangSelector').length == 0 ) return;
       ...
       if (nt_currency) {
          ...
          // /browsing_context_suggestions.json?source=geolocation_recommendation&country[enabled]=true&currency[enabled]=true&language[enabled]=true
          $.ajax({
             type: 'get',
      ...
      geckoShopify.QAllPages = function () {
          var timer = 300;
          //Question click
          if (designMode) {
            var selector1 = body,
                selector2 = '.group-question label';
          } else {
            var selector1 = $(".group-question"),
                selector2 = 'label';
          ...
          selector1.on('click', selector2, function() {
            ...
            if (_parent.hasClass("active")) {
              _parent.removeClass("active").find(".content").stop().slideUp(timer);
            } else {
               //console.log(_parent.siblings(".active"))
               _parent.siblings(".active").removeClass("active").find(".content").slideUp();
               _parent.addClass("active").find(".content").slideDown(timer);
            }
           });

          // // blogs page click title
          // $(".widget h5.widget-title").click(function(){
          //     $(this).toggleClass("check-active");
          //     $(this).parent().find(".widget-title ~ *").slideToggle(timer);
          // })
          // // blogs page mobile sidebar
          // $(".menu-mobile-block").click(function(){
          //     $("html").toggleClass("hside_opened");
          //     $("body").toggleClass("active-sidebar");
          //     $(".mask-overlay").toggleClass("mask_opened");
          // })
          ...
          //     var containermenu = $('.nt_single_blog .sidebar');
          //     if (!containermenu.is(e.target)  && !$(".menu-mobile-block").is(e.target)// if the target of the click isn't the container...
