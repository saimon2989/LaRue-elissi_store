   ...
   geckoShopify = (function() {
      ...
      return {
         ...
         */
         ...
         CustomFlickityT4: function (el) {
            el = el || $('[data-flickitycus]')
            if ( el.length == 0 ) return;
             var txtIsselected = 'is-selected',
                 IsSafari = (jscd.browser == "Safari");
             ...
             el.each(function() {
               ...
               var _this = $(this),
                   __this = $(this)[0],
                   timer = 400,
                   Isthumb = __this.hasAttribute("data-t4s-thumbX"),
                   IsthumbVertical = __this.hasAttribute("data-t4s-thumbY"),
                   option = JSON.parse(_this.attr("data-flickitycus") || '{}'),
                   typethumb = option.typethumb,
                   IdSlider = option.t4sid,
                ...
                // Initialize flickity https://codepen.io/desandro/pen/VLPJqO
                var $carousel = _this.flickity(option);

                var $carouselNavCells = $carouselNav.find('.t4s-carousel__nav--item'),
                   navHeightOld = $carouselNav.height(),
                   navCellHeight = navHeightOld/$carouselNavCells.length,
                   navHeight,navCellHeight,$selected,scrollX,scrollY;
                   //console.log('navCellHeight',navCellHeight)
               ...
               var flkty = $carousel.data('flickity');
                ...
                if (IsthumbVertical && window_w >= 1025) {

                  // Initialize Thumbnails Markup
                  initThumbnailsMarkup(_this,$carouselNav,typethumb,IdSlider);

                  $carouselNavCells = $carouselNav.find('.t4s-carousel__nav--item');

                  $carouselNav.addClass('t4s__thumbY--initialized');
                  navHeight = $carouselNav.height()
                  //navCellHeight = navCellHeight + 15;
                  $carousel.on( 'select.flickity', function(event, index) {
                    //console.log('select.flickity',$carousel);
                    // set selected nav cell
                    $carouselNav.find('.'+txtIsselected).removeClass(txtIsselected);
                    $selected = $carouselNavCells.eq( index ).addClass(txtIsselected);
                    // scroll nav
                    // console.log(IdSlider,$carouselNavCells)
                    ...
                    // console.log(navHeight, navCellHeight);
                    // console.log($selected.position().top, $selected.height());
                    // $carouselNav.animate({ scrollTop: scrollY });
                    // $carouselNav.stop().animate({
                    //     scrollTop: scrollY
                    // }, timer);
                    if (scrollY) {
                          //scrollY = $selected.position().top + navCellHeight;
                      //$carouselNav.scrollTop( scrollY );
                      if (IsSafari) {
                            $carouselNav.animate({ scrollTop: scrollY });
                      } else {
                        $carouselNav.scrollTop( scrollY );
                  ...
                  //$carouselNav.flickity(JSON.parse($carouselNav.attr("data-flickitycus")));
                  $carouselNav.addClass('t4s__thumbX--initialized');
                  $carousel.on( 'select.flickity', function(event, index) {
                    // set selected nav cell
                    $carouselNav.find('.'+txtIsselected).removeClass(txtIsselected);
                    $selected = $carouselNavCells.eq( index ).addClass(txtIsselected);
                    // scroll nav
                    //if ( slider_height <= navHeight) return;
                    scrollX = $selected.position().left + $carouselNav.scrollLeft() - ( navHeight + navCellHeight ) / 2;
                    //$carouselNav.animate({ scrollLeft: scrollX });
                // $carouselNav.stop().animate({
                //     scrollLeft: scrollX
                // }, timer);
                $carouselNav.scrollLeft( scrollX );
                  });
          ...
          function initThumbnailsMarkup(el, nav, type, id) {
            var markup = '',e_imgnv=true,
               el_length = el.find('.t4s-carousel__main--item:not(.t4s-dt-demo)').length;
               
            if (el_length == 0) return;
            if (el_length > 1) {
              $('.btn__prev--'+id+',.btn__next--'+id).addClass('t4s-show');
            }
            el.find('.t4s-carousel__main--item').each(function () {
              var $this = $(this),
                 _this = $this.find('.main-img'),
                 ...
                 style = '';
                  ...
                  if (type == '2') {
                    markup += '<div class="col-3 t4s-carousel__nav--item"><div class="item-deal-thumb"><div class="item-deal-img"><span class="nt_bg_lz lazyloadt4s" data-bgset="'+bgset+'" data-widths-none data-ratio="'+ratio+'" data-sizes="auto"'+style+'></span></div><div class="item-deal-info"><span class="item-deal-title">'+$this.find('.product-title').text()+'</span><span class="item-deal-price price">'+$this.find('.price').html()+'</span></div></div></div>';
                  } else {
                    markup += '<div class="col-3 t4s-carousel__nav--item"><span class="nt_bg_lz lazyloadt4s" data-bgset="'+bgset+'" data-widths-none data-ratio="'+ratio+'" data-sizes="auto"'+style+'></span></div>';
                  }
            });

            nav.empty().append(markup);
            body.trigger('refresh_currency');
          };
