    ...
    function updateGroupVariant(selector,opname,value,prefix,mdid) { 
      ...
      if ( $(parent+' [data-grname="not4"]').length == $(parent+" [data-grname]").length || $(parent+' [data-grname="'+opname+'"]').length == 0  ) return;
        ...
        var parent_pnav = '[data-featured-product-se][data-id="'+prefix+'"] .p-nav';
          ...
          //$(parent_pnav+'.flickity-enabled').removeClass('p-nav-ready');
  ...
  //bltimezone = false
    ...
    geckoShopify.delivery_order = function (selector) {
      ...
      if (selectorCurent.length == 0) return;
         ...
         //   //  //console.log('format: ',moment(timezoneDay).format('YYYY-MM-DD HH:mm:ss'))
         //   //  var lastDay = dayjs.tz(moment(timezoneDay).format('YYYY-MM-DD')+' '+time, timezone).toDate();
         //   // } else {
   ...
   //https://github.com/vimeo/player.js/#getloop-promiseboolean-error
